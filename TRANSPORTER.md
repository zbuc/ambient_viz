...
cloud of grains that **start their read at the current playhead and play
backward into the audio prior to it** (negative rate) — each grain is
sourced from points before the playhead but plays in reverse *at* the
playhead (the reverse read is what reaches back; there is no head-start
gap). Hann-windowed and overlapping, they sum into a sustained reversed
ghost of the recent material. "Polyphonic" = the overlapping grain cloud;
"pad" = the smooth reversed texture. Built on the
[`buffer_player`](daisy/crates/dsp/src/buffer_player.rs) substrate
(`CaptureBuffer` + `Grain` + `WindowTable`).

**Increment 1 (done):** the live-capture **effect** form — input captured
into the ring (write head = primary playhead), each grain spawns *at* the
playhead and reads backward into the prior audio, pad emitted as a parallel
send (like `Freeze`) for the engine to mix under the dry. Params:
`grain_ms`, `density`, `offset_ms` (optional pre-delay; 0 = at the
playhead), `pitch` (unity..+octaves), `reverse`, `spread` (length-scaled
spawn jitter), `level`. `no_std`, fixed `MAX_GRAINS` pool, no per-sample
transcendentals. The grain-cloud core is role-agnostic — a **played (MIDI,
per-note pitched) pad instrument** is the same scheduler over a per-voice
playhead + pitch, the natural next increment. Not yet Engine-wired.

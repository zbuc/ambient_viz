// Shared machinery for the phase-5 visualizer-mapping gates
// (validate-twist.js, validate-bitmap.js — one thin CLI per mapping, the
// per-mapping doctrine made physical).
//
// The legacy side of every visualizer mapping lived inside applyAutomation()
// and never crossed a tapped boundary, so there is no captured output trace
// to diff a graph against. The reference is therefore a FRAME-CLOCKED MODEL
// of the legacy browser math — the dt-clamped EMA (static/index.html,
// DISTANCE_SMOOTH_TAU_S) driven by the capture's `sse_out` stream (exactly
// the entries the page received), with each mapping supplying its shape as a
// callback. The candidate side is the real pipeline: capture inputs -> sim
// (manifests + policy + the mapping's graph) -> the fx.viz.* trajectory.
//
...
// same instant OR within the declared lag window, violating runs judged
// against the declared SETTLE-HOLD transient class. Declared per path in
// tools/replay/tolerances.js → derived.
...
// The legacy browser model — applyAutomation()'s sensor conditioning,
// verbatim semantics: per-frame dt-clamped EMA over the latest distance
// ([0, 500] ms, seed on first sample) and the endpoint guards (near >= 0,
// far > effective near, defaults 75/130). `shapeFn(smoothed, nearCm, farCm)`
// supplies the mapping's value — including its sensor-absent default
// (smoothed === null until the first distance arrives).
function legacyVizModel(events, shapeFn, { tauS = 0.25 } = {}) {
  const entries = [];
  ...
  for (let t = t0; t <= tEnd + FRAME_MS; t += FRAME_MS) {
    ...
    const value = shapeFn(smoothed, nearCm, farCm);
    if (value !== lastValue) { out.push({ t, value }); lastValue = value; }
  }
  return out;
}
...
// The full per-mapping gate: sim the graph, model the legacy math, compare,
// and (when the capture carries them) demand the live bus_tx value-change
// sequence equals the simulated one exactly — same rule as the tape gate.
function validateVizMapping({ goldenDir, graphFile, outDir, quiet = false,
  busPath, label, shapeFn }) {
  ...
  const trajectory = collected[busPath] || [];
  const golden = loadSession(goldenDir);
  const legacy = legacyVizModel(golden.events, shapeFn);
  const tol = tolerances.derived[busPath];
  ...
  let live = { present: false, note: `capture has no bus_tx for ${busPath} (recorded before this graph ran live)` };
  if (busTx.length) {
  ...
  fs.writeFileSync(path.join(outDir, `${label}-validation.json`), JSON.stringify(validation, null, 2));

  if (!quiet) {
    console.log(`${label} graph: ${trajectory.length} ${busPath} points vs `
      + `${legacy.length} legacy-model changes`);
    console.log(`  ${model.verdict.padEnd(20)} model:${busPath}  `
    ...
    if (live.present) {
      console.log(`  ${(live.pass ? 'MATCH' : 'REGRESSION').padEnd(20)} live:${busPath}  `
        + `${live.live_changes} live changes vs ${live.sim_changes} sim (${live.live_publishes} live publishes)`);
    } else {
      console.log(`  ${'ABSENT'.padEnd(20)} live:${busPath}  ${live.note}`);
    }
    console.log(`\nverdict: ${validation.verdict}  (report: ${path.join(outDir, `${label}-validation.json`)})`);
  }
  return { validation, simReport: report };
}

// Shared CLI shell for the thin per-mapping gates.
function runGateCli({ argv, defaultGraph, busPath, label, shapeFn, usage }) {
  const args = { dir: null, graph: defaultGraph, out: null, quiet: false };
  ...
  const { validation } = validateVizMapping({
    goldenDir, graphFile: args.graph, outDir, quiet: args.quiet, busPath, label, shapeFn,
  });
  process.exit(validation.blocks ? 1 : 0);
}

module.exports = { legacyVizModel, gridSample, compareDerived, validateVizMapping, runGateCli };
